...
def sauvegarde_liste(animes):
    ...
    Args:
        animes (list): Liste des objets Anime
    """
    choix_sauvegarde = ""
    # Boucle de choix
    while choix_sauvegarde != "Y" and choix_sauvegarde != "N":
        ...
        if choix_sauvegarde == "Y":
            anime_dataframe = create_dataframe_liste(animes)
            sauvegarde_csv(anime_dataframe)


def create_dataframe_liste(animes):
    """Cette fonction génère un dataframe à partir de la liste des objets Anime
    ...
    anime_dataframe = pd.DataFrame(
        [[anime.nom, anime.id, anime.score, anime.etat] for anime in animes],
